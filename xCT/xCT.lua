    ...
    if event == "COMBAT_TEXT_UPDATE" then
        ...
        if SHOW_COMBAT_TEXT == "0" then
            ...
            if subevent == "DAMAGE" then
                xCTdmg:AddMessage("-"..arg2, 1, 0, 0)
                
            elseif subevent == "DAMAGE_CRIT" then
                xCTdmg:AddMessage(ct.critprefix.."-"..arg2..ct.critpostfix, .9, .3, .09)
                ...
                if arg3 >= ct.healtreshold then
                    if arg2 then
                        if COMBAT_TEXT_SHOW_FRIENDLY_NAMES == "1" then
                            xCTheal:AddMessage(arg2.." +"..arg3, .7, .9, .1)
                        else
                            xCTheal:AddMessage("+"..arg3, .7, .9, .1)
                        end
                    end
                end
            ...
            elseif subevent == "HEAL_CRIT" then
                if arg3 >= ct.healtreshold then
                    if arg2 then
                        if COMBAT_TEXT_SHOW_FRIENDLY_NAMES == "1" then
                            xCTheal:AddMessage(arg2.." +"..arg3, .17, .98, .02)
                        else
                            xCTheal:AddMessage("+"..arg3, .17, .98, .02)
                        end
                    end
                end
                return
...
for i = 1, numf do
    ...
    if framenames[i] == "dmg" then
        ...
        f:SetPoint("CENTER", -224.5, -42)
    elseif framenames[i] == "heal" then
        f:SetJustifyH(ct.justify_2)
        f:SetPoint("CENTER", -329, -42)
        f:SetWidth(128)
    elseif framenames[i] == "gen" then
        f:SetJustifyH(ct.justify_3)
        f:SetWidth(256)
		f:SetHeight(50)
        f:SetPoint("CENTER", -3, -105)
		f:SetFont(ct.font, 8, "OUTLINEMONOCHROME")
...
local StartConfigmode = function()
    if not InCombatLockdown() then
        for i = 1, #ct.frames do
            ...
            if framenames[i] == "dmg" then
                f.fs:SetText(DAMAGE)
                f.fs:SetTextColor(1, 0, 0, .9)
            elseif framenames[i] == "heal" then
                f.fs:SetText(SHOW_COMBAT_HEALING)
                f.fs:SetTextColor(.7,.9,.1,.9)
            elseif framenames[i] == "gen" then
