...
cargBags_Nivaya:RegisterEvent("UI_SCALE_CHANGED")
...
function cargBags_Nivaya:ADDON_LOADED(event, addon)
	...
	cargBags_Nivaya:UI_SCALE_CHANGED()
end

function cargBags_Nivaya:UI_SCALE_CHANGED()
	local scale = 768 / string.match(({GetScreenResolutions()})[GetCurrentResolution()], "%d+x(%d+)")
	local curScale = UIParent:GetEffectiveScale()
	for _,v in pairs(cB_Bags) do v:SetScale(scale / curScale) end
end
...
local function HandleSlash(str)
	...
	elseif str == 'consumables' then
		...
		cB_filterEnabled["Consumables"] = cBnivCfg.Consumables
		StatusMsg('The "Consumables" filter is now ', '.', cBnivCfg.Consumables, true, false)
