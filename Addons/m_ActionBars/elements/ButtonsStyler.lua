...
local function ActionButtons(self)
    ...
    ho:SetFont(cfg.mAB.media.button_font, cfg.buttons.hotkey_font_size, "MONOCHROMEOUTLINE")
	ho:ClearAllPoints()
	ho:SetPoint("TOPRIGHT")
	-- show/hide macro name, adjust font
    if not cfg.buttons.hide_macro_name then
		if mn then mn:SetFont(cfg.mAB.media.button_font, cfg.buttons.name_font_size, "MONOCHROMEOUTLINE") end
    else
		if mn then mn:Hide() end
    end
    co:SetFont(cfg.mAB.media.button_font, cfg.buttons.count_font_size, "MONOCHROMEOUTLINE")
	SetTextures(self, 1)
