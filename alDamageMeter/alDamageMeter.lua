...
local CreateBar = function()
	...
	newbar.left:SetPoint("LEFT", 5, 0)
	newbar.left:SetJustifyH("LEFT")
	
	newbar.right = CreateFS(newbar)
	newbar.right:SetPoint("RIGHT", -0, 0)
	newbar.right:SetJustifyH("RIGHT")
	
	newbar.right2 = CreateFS(newbar)
	newbar.right2:SetPoint("RIGHT", -32, 0)
	newbar.right2:SetJustifyH("RIGHT")
	newbar.left:SetPoint("RIGHT", newbar.right2, "LEFT", -10, 0) --right point of name is left point of value
	
	newbar.bg = CreateFrame("Frame", nil, newbar)
	newbar.bg:SetPoint("TOPLEFT", -1, 1)
	newbar.bg:SetPoint("BOTTOMRIGHT", 1, -1)
	newbar.bg:SetFrameLevel(newbar:GetFrameLevel()-1)
	
	newbar.bg:SetBackdrop(backdrop)
	newbar.bg:SetBackdropColor(0,0,0,0.5)
	newbar.bg:SetBackdropBorderColor(0,0,0,1)
...
local UpdateBars = function()
	...
	for i = 1, #barguids do
		...
		color = CUSTOM_CLASS_COLORS[cur.class]
		...
		if sMode == DAMAGE or sMode == SHOW_COMBAT_HEALING then
			bar[i].right2:SetText(numFormat(cur[sMode].amount))
			bar[i].right:SetText(numFormat(perSecond(cur)))
		else
			bar[i].right2:SetText("")
			bar[i].right:SetText(numFormat(cur[sMode].amount))
		end
